 ...
 * Author: Prabhu
 *
 * Created on 12 January, 2026, 8:47 PM
 */
...
unsigned char preheat_sec = 0;
unsigned char preheat_done = 0;
unsigned char preheat_active = 0;
unsigned char cooking_started = 0;
int reset_flag = RESET_NOTHING;
int operation_flag = POWER_SCREEN; // power on screen
 ...
 void init_config(void) {
    ...
    HEATER_DDR = 0;   // RC5 output
    HEATER = 0;       // Heater OFF initially
...
{
    clcd_print("TIME =       ", LINE1(0));  // 16 chars
    ...
    if (sec == 0 && min == 0 && cooking_started)
...
void set_temp(unsigned char key, int reset_flag)
{
    ...
    /* Temp entry */
    if ((key != '*') && (key != '#') && (key != RELEASED) && !preheat_active)
    {
        if (key_count < 3)
        {
            key_count++;
    ...
    /* Start pre-heating */
    else if (key == '#' && temp <= MAX_TEMP && !preheat_active)
    {
        clear_screen();
    ...
    /* Display countdown (NO BLINK) */
    if (preheat_active)
    {
        clcd_ch((sec / 10) + '0', LINE3(12));
        clcd_ch((sec % 10) + '0', LINE3(13));
    }
    else
    {
        /* Blink temp entry */
